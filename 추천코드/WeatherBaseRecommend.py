...
from img_path import load_closet, get_image_paths, get_dir_paths
...
def main(temperature, recommendDto: List[RecommendRequestDto]):
    ...
    base_dir_path = "스타일 코디 모음집/"    # 최신 트렌드 반영 모델 사진 (풀착장)
    ...
    base_image_paths = []
    image_dir_paths = get_dir_paths(base_dir_path)
    for image_dir_path in image_dir_paths:
        base_image_paths.append(get_image_paths(image_dir_path))
